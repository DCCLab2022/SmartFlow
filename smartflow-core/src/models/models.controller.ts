...
models.post('/models/:bundleId', (req, res) => {
    if (verifyAddress(req.body.caseCreator, 'Case Creator', res) && processRegistryContract !== undefined) {
      ...
      repoSchema.find({_id: req.params.bundleId},
                ...
                // AccessControlContract.new(processRegistryContract.address, repoDataPolicy[0].address, repoDataTaskRole[0].address,
                    ...
                    // (err, contract) => {
                        ...
                        // } else if (contract.address) {                                                   
                          ...
                          processRegistryContract.newBundleInstanceFor(repoData[0]._id.toString(), 0, "0xE4a629441C985603694F30Eb7Dde9021e5f2c8b6", {
                              from: web3.eth.accounts[executionAccount],
                              gas: 4500000
                          },
                          (errNew, resNew) => {
        ...
        console.log("process model not found");
        res.status(404).send('Process model not found');
});
...
export let findParametersOfVerificationFunc = (index: Number):String => {
  ...
  const atcontract = VerificationManagerContract.at("0xA3b79e9156374488568bfa0CDdd105Ac684BB20e")
  ...
  const testresult = atcontract.registerEachTaskMetadata.sendTransaction("test01",0x00,"test02","test03","string name",txObject);
  ...
  let parameterString: String = atcontract.getParametersOfTaskMetadatas.call(index);
...
let findParameters = (contractAbi, functionName) => {
    ...
    console.log("contractAbi, functionName");
