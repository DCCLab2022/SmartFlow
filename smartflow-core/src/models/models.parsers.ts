...
import {findParametersOfVerificationFunc, findArgsOfVerificationFunc, findSigOfVerificationFunc} from './web3Configuratoin';
// import {findParametersOfVerificationFunc} from './models.controller';
        ...
        parseBpmn(modelInfo.bpmn)
            .then((definitions: any) => {
                ...
                globalControlFlowInfo.forEach(controlFlowInfo => {
                  ...
                  // console.log("controlFlowInfo");
                  // console.log(controlFlowInfo.self.flowElements);

                  // console.log('eeeeeeeeeeeeeeeee');
                  const infoObj = controlFlowInfo.self.flowElements;


                    // console.log(controlFlowInfo.self.flowElements[1].$attrs.attr1);
                    // console.log(controlFlowInfo.self.flowElements[1].$attrs['task-smart-contract']);

                  for (let e of infoObj) {
                    // console.log("e");
                    // console.log(e);
                    const attrs = e.$attrs;
                    // console.log("attrs");
                    ...
                    if(Object.keys(attrs).length !== 0){
                      ...
                      const argsMap = controlFlowInfo.accessContractArgsMap;
                      const taskMap = controlFlowInfo.accessContractMap;
                      
                      taskMap.set(e.id, attrs["verification-function"]);
                      
                      // console.log('if attrs');
                      
                      // console.log(attrs["task-smart-contract"]);
                      // console.log(attrs.attr1);

                      // console.log('attrs end');

                      // console.log(typeof attrs.attr1);
                      // console.log(attrs.attr1);

                      if(attrs.attr1 !== undefined){
                        argsMap.set(e.id, new Array<string>());
                        let temp = argsMap.get(e.id);
                        argsMap.set(e.id, [...temp, attrs.attr1]);

                        // console.log(typeof attrs.attr2);
                        // console.log(attrs.attr2);

                        if(attrs.attr2 !== undefined){

                          temp = argsMap.get(e.id);
                          argsMap.set(e.id, [...temp, attrs.attr2]);

                          if(attrs.attr3 !== undefined){

                            temp = argsMap.get(e.id);
                            argsMap.set(e.id, [...temp, attrs.attr3]);

                            if(attrs.attr4 !== undefined){
                              temp = argsMap.get(e.id);
                          
                              argsMap.set(e.id, [...temp, attrs.attr4]);
                            }
                          }
                        }
                      }
                ...
                console.log("mainControlFlowInfo");
                console.log(mainControlFlowInfo);

                // console.log("globalControlFlowInfoMap");
                // console.log(globalControlFlowInfoMap.self.flowElements);
                ...
                for (let controlFlowInfo of globalControlFlowInfo) {
                    if (!controlFlowInfo.isEmbedded) {
                        ...
                        let workListGenerationInfo = {
                            ...
                            getContracts: () => {
                              let map = new Map();
                              const argsMap = controlFlowInfo.accessContractMap;
                            ...
                            getContractsArgs: () => {
                                let map = new Map();
                                const taskMap = controlFlowInfo.accessContractArgsMap;
                                taskMap.forEach((val, key) => {
                                    map.set(key, val);
                                });
                                return map;
                            },
                            findParametersVerificationFunc: (idx: String):String => {
                              let str: String = findParametersOfVerificationFunc(idx);
                              return str;
                            },
                            findArgsVerificationFunc: (idx: String):String => {
                              let str: String = findArgsOfVerificationFunc(idx);
                              return str;
                            },
                            findSigVerificationFunc: (idx: String):String => {
                              let str: String = findSigOfVerificationFunc(idx);
                              return str;
                            },
                            is: is
                        };
